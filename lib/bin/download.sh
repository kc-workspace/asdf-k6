...
__asdf_bin() {
  ...
  if kc_asdf_is_ref; then
    url="${ASDF_OVERRIDE_REF_REPO:-https://github.com/grafana/k6.git}"
    ...
    url="https://github.com/grafana/k6/releases/download/v{version}/k6-v{version}-{os}-{arch}.{ext}"
    url="$(kc_asdf_template "$url" "${vars[@]}")"
  ...
  if [[ "$mode" == "custom" ]]; then
    ...
    if kc_asdf_enabled_feature checksum; then
      local checksum_url
      checksum_url="https://github.com/grafana/k6/releases/download/v{version}/k6-v{version}-checksums.txt"
    ...
    if [[ "$mode" == "file" ]]; then
      ...
      local internal_path
      outpath="$outdir"
      internal_path="k6-v{version}-{os}-{arch}"
      [ -n "$internal_path" ] &&
